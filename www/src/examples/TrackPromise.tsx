...
export function TrackPromise() {
  ...
  return (
    <Panel>
      ...
      />
      <Row>
        <Btn onClick={() => progress.track(delay(1500))}>Fetch (1.5s)</Btn>
        <Btn onClick={() => progress.track(fail(1200)).catch(() => {})}>Fail it (1.2s)</Btn>
        <Btn onClick={() => progress.track(delay(120))}>Too fast to show (120ms)</Btn>
      </Row>
      <Readout>
